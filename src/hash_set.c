...
{
    hash_set new_hs = hash_set_create(hs_bucket_vector_size(hs)*2);
...
{
    hs_bucket b = hs_bucket_vector_get(hs, hash_code_modulo(string, hs_bucket_vector_size(hs), *hash_fct));
...
hash_set hash_set_add(hash_set hs, const char *string, size_t (*hash_fct)(const char *str))
{
    ...
    if (hs_elt_vector_size(b) > HASH_SET_BUCKET_MAX_SIZE)
